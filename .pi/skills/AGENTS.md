...
| `scenario-design/references/test-plan-schema.md` | Exact `test-plan.md` layout. Header: stage + generated date. Soft-gate clarification banner. Now carries mandatory `disposition` column (`automated`\|`manual-only`) per row — manifest is source of truth for automated-vs-manual. → see `scenario-design/references/test-plan-schema.md.AGENTS.md`. See change: add-openspec-pipeline-orchestrators |
| `scenario-design/SKILL.md` | Drafts adversarial real-life test scenarios from OpenSpec change spec. Phase 4 gains additive `manual-only` routing outcome (aesthetics/hardware/subjective, level —); Phase 5 emits `disposition` per row; routing footnote reads dynamic docker harness port from `.pi-test-harness.json` (not `:18000`). → see `scenario-design/SKILL.md.AGENTS.md`. See change: add-openspec-pipeline-orchestrators |
