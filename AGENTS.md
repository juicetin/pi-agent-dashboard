...
| `src/extension/ask-user-tool.ts` | `ask_user` tool registration (bundled in bridge, registered at session_start to avoid static tool-name conflicts with other extensions). `multiselect` dispatches through `polyfillMultiselect` (since pi-coding-agent's `ExtensionUIContext` has no `multiselect` method). Tool description instructs agents: *"UI provides a Select all toggle; do not add one."* **Schema shape (commit a53933f)**: `parameters` is a single flat `Type.Object` (root `type: "object"`) — NOT a `Type.Union` of per-method arms — because OpenAI's function-calling validator rejects root-level `anyOf` with *"schema must be a JSON Schema of 'type: \"object\"'"*. All fields except `method` are `Type.Optional`; per-method shape requirements (select/multiselect need `options`, batch needs `questions[]`, etc.) are enforced **at runtime** by `prepareArguments` (rescue/normalization: `params`-wrapping unwrap, `question`→`title`, stringified `options`/`questions` parsing, batch `method` synthesis from `questions[]`, batch `title` backfill from first sub-question) and the `execute` switch. Sub-questions use `SubQuestionSchema` (also flat object) whose `method` Union deliberately omits `"batch"` (no nesting). See changes: ask-user-multiselect-polyfill, refactor(schema)-restructure-ask-user-tool-schemas. |
...
| `src/client/lib/event-reducer.ts` | Event-sourced state reducer (delegates flow events to flow-reducer); extracts LLM errors from `agent_end` into `lastError`. **Assistant content-array reorder** (change: fix-text-tool-render-order): on every `message_end` for `role:"assistant"`, the pure helper `reorderToolCardsForAssistantMessage(messages, content)` reorders the trailing K rows so that text/thinking/toolCall blocks land in the same order as the model's `content[]` array — fixes the case where Anthropic-style `[text, toolCall]` messages render the tool card above their own assistant text because `tool_execution_start` pushes the running tool bubble before the streaming text reaches `message_end`. The reorder matches `text` → most-recent `assistant` row, `toolCall` → `toolResult` by `toolCallId`, `thinking` → most-recent `thinking` row; preserves React keyed reconciliation (`tool-${toolCallId}` ids unchanged). **Unclaimed-row guard**: when the K-sized suffix window slurps a prior-message row (e.g. an empty `thinking` block in the content array, or a `tool_execution_start` that hasn't fired yet), unclaimed suffix rows keep their **original suffix index** rather than being appended at the tail — prevents prior `user` / `assistant` rows from migrating past the just-ended message. Replay path inherits the fix because it routes through the same reducer. |
