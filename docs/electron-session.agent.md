...
- Phase 4: macOS Catalina Support — 10.15 VMware. Failed: `LSMinimumSystemVersion` plist (Mach-O `minos 11.0` baked). Worked: Electron 33→32 (last 10.15 support). Superseded: floor 10.15→12.0 (change `upgrade-electron-runtime`). Below-floor users remain on currently-installed build.
