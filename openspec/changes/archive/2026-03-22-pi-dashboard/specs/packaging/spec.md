...
The package SHALL declare pi core packages as peer dependencies with `"*"` range: `@mariozechner/pi-coding-agent`, `@sinclair/typebox`, `@mariozechner/pi-tui`, `@mariozechner/pi-ai`.

Runtime dependencies (React, better-sqlite3, ws, etc.) SHALL be in `dependencies`.

#### Scenario: Peer dependency resolution
- **WHEN** the package is installed via pi
- **THEN** pi core packages SHALL be resolved from pi's own installation, not bundled
