...
The MarkdownContent component SHALL accept a `content` string prop and render it as formatted HTML using react-markdown with the `remark-gfm` plugin enabled. Supported elements SHALL include: paragraphs, headings, bold, italic, strikethrough, lists (ordered and unordered), links, inline code, fenced code blocks, GFM tables, task lists, autolinks, and blockquotes.
