# Spec Delta — session-card-status
...
### Requirement: Unread sessions display the gray-stripes pulse
...
#### Scenario: Unread alive session shows gray stripes
...
#### Scenario: Streaming-and-unread session prefers yellow over gray
...
#### Scenario: ask_user-and-unread session prefers purple over gray
...
#### Scenario: Ended-and-unread session still shows gray stripes
...
1. A **45° barber-pole stripe pattern** in low-alpha amber, drifting **purely horizontally** by translating `background-position` linearly along the X axis only.
2. A **breathing pulse** in which the overall element opacity oscillates smoothly.

The stripe `background-size` SHALL be set to exactly one diagonal pattern period (`28.2843px × 28.2843px`, i.e. `20√2`) so tiles repeat seamlessly. The animation SHALL translate by an integer number of full periods for a perfectly seamless loop.

The combined intensity SHALL remain ambient (low alpha, low contrast) so streaming cards are clearly distinguishable from idle cards without being visually loud.
...
**Precedence (modified)**: `card-working-pulse` SHALL take priority over `card-unread-pulse`. A session that is both streaming AND has `unread: true` SHALL render only the yellow streaming pulse; the unread bit remains set in state but is not visualized while streaming, and re-emerges as gray stripes when the session transitions back to a quiescent state if still unviewed.

**Idle scenario (modified)**: an idle session card SHALL have no pulse class **only when** `unread === false`. An idle session card with `unread === true` SHALL have the `card-unread-pulse` class per the new Unread requirements above.

#### Scenario: Streaming session card has stripes and pulse

- **WHEN** a session enters `streaming` status
- **THEN** the rendered card element SHALL have the `card-working-pulse` class
- **AND** the computed background SHALL include a repeating linear gradient at 45°
- **AND** an animation SHALL translate `background-position` along the X axis (horizontal drift only)
- **AND** an animation SHALL oscillate the element opacity
...
#### Scenario: Streaming-then-idle with no view restores gray stripes
