...
When `autoInitWorktreeOnSpawn` is `true`, after a successful worktree spawn the
client SHALL probe init-status for the new checkout and auto-invoke the existing
init flow ONLY when the hook is trusted and `needsInit` is true.
...
When `autoInitWorktreeOnSpawn` is `true` but the hook is untrusted, the client
SHALL NOT auto-invoke the init flow. The `WorktreeInitButton` SHALL appear so the
user can grant TOFU trust manually.
