...
- [x] 0.1 Serve `openspec/changes/non-disruptive-file-open/mockups/` and open
...
- [x] 0.2 Confirm sequencing with `redesign-split-layout-controls` (proposal
...
- [x] 1 Add `activate?: boolean` to the `openFile` action (default `true`).
- [x] 1 Add `unread?: boolean` to `OpenFile`.
- [x] 1 `openFile` with `activate === false`: NEW tab → push, keep `activeIndex`,
...
- [x] 1 Enforce the invariant **the active tab is never unread**: clear `unread`
...
- [x] 1 `isValidState` tolerates the optional `unread` field (older blobs stay
...
- [x] 1 Unit tests: activate:false adds without moving active + sets unread;
...
- [x] 2 Extract `ensureRevealed()` → `if (split.mode === "closed")
...
- [x] 2 Route the param-less deep-link mode transition in `SessionSplitView.tsx`
  (`else updateSplit({ mode: "split" })`) through `ensureRevealed()` so a deep-link
  from `full` does not yank to `split`.
- [x] 2 Add `opts?: { background?: boolean }` to **all three** canvas openers
...
- [x] 2 Tests: closed→split reveal; full stays full; split stays split;
...
- [x] 2 **Rewrite the existing `F9` test** in `SplitWorkspaceContext.test.tsx` —
...
- [x] 3 `CanvasDriver.tsx`: give `useOpenTarget` a `background` param. The
  ...
  (file / live / url).
- [x] 3 Confirm foreground (no background flag) for the user-click paths:
...
- [x] 3 Leave `EditorPane.tsx` `live:preview` direct `openFile` dispatch as-is
  (mode-preserving in-pane action; documented exception).
- [x] 3 Test: an auto-canvas **effect** open while another tab is active leaves
...
- [x] 4 Render an unread dot on tabs where `unread === true`.
- [x] 4 Play a one-time highlight/pulse when a tab is added OR re-signalled in the
...
- [x] 4 Any activation (click, keyboard, or `closeTab` re-activation) clears the
  unread dot — the active tab never shows a dot.
- [x] 4 Test: unread dot renders for a background tab and disappears after
...
- [x] 5.1 Run the client in dev; compare to `mockups/index.html` in dark AND light
  — mode stickiness, silent background add, unread dot, one-time pulse.
- [x] 5.2 `npm run quality:changed` (Biome + tsc + tests) green.
- [x] 5.3 Code-review gate on the diff (`review-changes.ts`); fix Critical/Warning.

## 6. QA / manual

- [x] 6.1 In `full` mode, click a file in the tree → stays `full`, file activates.
- [x] 6.2 In `split` mode reading `a.ts`, have the agent write `b.ts` → `a.ts` stays
  active, `b.ts` arrives unread with a pulse; click `b.ts` → dot clears.
- [x] 6.3 In `closed` mode, agent writes `b.ts` → split reveals with `b.ts` shown.
- [x] 6.4 Mobile viewport: agent write shows the tap-to-open chip, no pane yank;
  tapping the chip activates the tab (foreground).
- [x] 6.5 Header Changed-Files chip from `full` → mode stays `full`, the Changes/tree
  rail expands inside the editor with chat hidden (sign off the intentional visual).
- [x] 6.6 Deep-link `/session/:id/editor` (no file param) from `full` → stays `full`.
...
- [x] 7 `activate:false` + tab-not-open → pushed, activeIndex unchanged, new tab
  `unread:true` (test-plan #E1).
- [x] 7 `activate:false` + open-inactive tab → activeIndex unchanged, tab set
  `unread:true` (re-signal) (test-plan #E2).
- [x] 7 `activate:false` + open-active tab → no-op, active stays `unread:false`
  (test-plan #E3).
- [x] 7 `activate` omitted → tab activated, `unread` unset (back-compat)
  (test-plan #E4).
- [x] 7 Invariant: `closeTab` re-points activeIndex onto an unread adjacent tab →
  that tab's `unread` cleared (test-plan #E5).
- [x] 7 Persisted blob without `unread` loads valid (test-plan #E6).
- [x] 7 Persisted blob `unread:42` rejected → EMPTY_PANE_STATE + log
  (test-plan #E7).
- [x] 7 Background `unread:true` tab, not activated → save+load keeps it unread
  (test-plan #E8).
...
- [x] 7 `closed` + `openInSplit('a.ts')` → mode `split`, `a.ts` active
  (test-plan #F1).
- [x] 7 `full` + `openInSplit('a.ts')` → mode stays `full`, `a.ts` active
  (test-plan #F2).
- [x] 7 `split` + `openInSplit('a.ts')` → mode stays `split` (test-plan #F3).
- [x] 7 `full` + `openChanges()` → mode stays `full` (rewrites F9) (test-plan #F4).
- [x] 7 `split`/`full` reading `a.ts` + user `openInSplit('b.ts')` → `b.ts`
  active, NOT unread (test-plan #F5).
- [x] 7 `split`/`full` reading `a.ts` + `openInSplit('b.ts',{background:true})` →
  `a.ts` stays active, `b.ts` unread, mode unchanged (test-plan #F6).
- [x] 7 `closed` + background open → mode `split`, target active, NOT unread
  (test-plan #F7).
- [x] 7 `split` + `openLiveTarget(url,{background:true})` → live tab unread,
  `a.ts` active (test-plan #F8).
- [x] 7 `split` + `openUrlTarget(url,{background:true})` → url tab unread,
  `a.ts` active (test-plan #F9).
- [x] 7 Background open with `line=20` → no `pendingScroll` stashed
  (test-plan #F10).

L1 tab-strip render — extend `packages/client/src/components/editor-pane/__tests__/EditorPane.test.tsx`:

- [x] 7 Unread dot renders on a background tab; gone after click-activation
  (test-plan #F16).
- [x] 7 Repeat background open of an open-unread tab re-pulses, stays unread +
...
- [x] 7 `full` + navigate `/session/:id/editor` (no file) → stays `full`
  (test-plan #F11).
- [x] 7 desktop `split` reading `a.ts` + agent canvas → `b.ts` → `a.ts` stays
  active, `b.ts` unread + dot, mode unchanged (test-plan #F12).
- [x] 7 desktop `closed` + agent canvas → mode `split`, target active
  (test-plan #F13).
- [x] 7 mobile (<768w) canvas change → no yank, `canvas-file-chip` visible
  (test-plan #F14).
- [x] 7 mobile chip tap → target opens active, NOT unread (test-plan #F15).
- [x] 7 `prefers-reduced-motion: reduce` + background tab → dot shows, no pulse
  animation (test-plan #F19).

## 8. Manual-only scenarios (deferred post-merge by ship-change)

- [x] 8.1 Pulse smoothness / one-shot feel — human watches a background tab arrive;
  pulse plays once, non-jarring, draws the eye without stealing focus
  (test-plan: manual-only).
- [x] 8.2 Changed-Files chip from `full` — Changes/tree rail expands inside the
