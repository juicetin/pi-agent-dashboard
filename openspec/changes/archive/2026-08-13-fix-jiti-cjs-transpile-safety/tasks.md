...
- [x] 1.1 Add `scripts/__tests__/jiti-cjs-transpile-safety.test.mjs`. Discover
...
- [x] 1.2 Detection is **AST-level** — parse the emitted module, locate
      `MetaProperty` nodes. Do **not** substring/regex the emitted text.
- [x] 1.3 Exclusions, each with a comment saying why: Vite/client source
...
- [x] 1.4b **Seed 2 — the dashboard server itself.** Key on a workspace whose
...
- [x] 1.4b2 **Seed 3 — `pi-dashboard-plugin` `server` / `bridge` entries.** Read
      ...
      four `bridge`).
- [x] 1.4b3 The walk follows ESM `import` **and** CJS `require()` specifiers
...
- [x] 1.4b4 Assert raw-`.ts` `bin` entries are **excluded** unless reached by a
...
- [x] 1.4c Confirm `packages/client-utils` stays out: re-run the check that no
      ...
      current answer.
- [x] 1.5 Assert the discovered set is **non-empty** and fail with a
      discovery-specific message when it is not. Do **not** add a hardcoded
      count floor.
- [x] 1.6 Fixtures under `scripts/__tests__/fixtures/`: (a) cast-wrapped
...
- [x] 1.7 Assert the two real **erasable-shape** files stay green:
...
- [x] 2.1 Build fixture `node_modules` trees for the three shapes where the
      resolvers disagree: (i) no `exports`, `module` + `main`; (ii)
      `exports["."]` nesting `node` / `default`; (iii) `exports` with subpaths
      but no `"."`.
- [x] 2.2 Assert for each: when the dir-walk returns a path, that path is what
      `defaultResolveModule` returns — i.e. the ESM step never overrides it.
- [x] 2.3 Assert shape (iii) — where the dir-walk succeeds via `main` and ESM
      would throw — is unaffected.
- [x] 2.3a Assert the guard is inert: with all three fixture shapes installed,
...
- [x] 2.3b Pin the two preconditions that make 2.3a true, so a future change
...
- [x] 2.4 Pin the live-registry invariant: `@earendil-works/pi-coding-agent` and
...
- [x] 2.5 Exercise these through a **jiti-loaded** `strategies.ts`, not a
...
- [x] 3.1 Reorder `defaultResolveModule` to `createRequire → dir-walk → ESM
      resolve`.
- [x] 3.2 Replace the ESM step with a direct `import.meta.resolve(id)` call
      inside a `try`; delete the `metaResolve` binding and the `typeof` guard.
      Keep the `file:` scheme check and `fileURLToPath` conversion.
- [x] 3.3 Rewrite the ordering doc-comment. It MUST say: (a) the ESM step is an
...
- [x] 3.3a Record the pre-existing defect without fixing it: the entry falls back
...
- [x] 3.4 In the same comment, correct two standing falsehoods: the synchronous
...
- [x] 3.5 Re-run 1.1, 1.9, and all of §2 — green.
- [x] 3.6 Run the existing `tool-registry` suite unchanged. Green is necessary
      but **not sufficient** (see §2 note); it only rules out a gross break.
- [x] 3.7 `npm run lint` (`tsc --noEmit`) — the direct call must type-check with
      no cast and no ambient declaration.

## 4. Verify the whole graph

- [x] 4.1 Transpile the full bridge-reachable import graph through jiti; assert
...
- [x] 4.3 Confirm gate runtime is acceptable in the default `npm test` run;
...
- [x] 5.1 Spawn `DocScribe` for the `docs/faq.md` entry: keyed on **both**
...
- [x] 5.2 Apply DocScribe's returned tree rows; never edit `docs/` directly.
- [x] 5.3 `packages/shared/src/tool-registry/AGENTS.md` — `strategies.ts` row
      gains `See change: fix-jiti-cjs-transpile-safety`.
- [x] 5.4 Add a `scripts/__tests__/AGENTS.md` row for the new gate.
- [x] 5.5 `kb dox lint` clean.

## 6. Review + land

- [x] 6.1 `doubt-driven-review` **cycle 2** on the rewritten artifacts. Cycle 1
...
- [x] 6.2 `review-code` on the diff once vitest is green.
- [x] 6.3 `npm run quality:changed`.
- [x] 6.4 Full `npm test` green (pipe once to `/tmp/`, then grep — do not rerun
      to inspect errors).
