...
- [ ] L1 D3a — absolute-under-cwd yields relative key + dedups (test-plan #D3a). input: absolute `${cwd}/src/foo.ts` + relative `src/foo.ts` from two sources · trigger: normalizePath+dedup · observable: key `src/foo.ts`, one entry. see `session-diff.test.ts`
- [ ] L1 D3b — sibling-prefix dir not admitted (test-plan #D3b). input: cwd `/home/user/project`, candidate `/home/user/project-backup/x.ts` · trigger: containment check · observable: excluded (relative gives `../project-backup/x.ts`). see `session-diff.test.ts`
