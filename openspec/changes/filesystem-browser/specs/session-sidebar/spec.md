...
When `session.isWorktree` is `true`, the session card SHALL show the worktree folder name (last segment of `session.cwd`) with a tree icon (🌲) instead of the branch name with a branch icon. The group header SHALL continue showing the branch name as before.

Group headers SHALL display action buttons (localhost only) alongside editor buttons:
- "Add pi-agent" button — spawns a new pi session in the group's CWD
- "Add worktree" button — opens a dialog to create a git worktree (only shown when group has git branch info)

The sidebar SHALL merge session-derived groups with DB-persisted workspaces. Empty workspaces (0 sessions) SHALL appear as groups with action buttons. DB-backed groups SHALL show a "✕ remove" button.

An "Add Folder" button SHALL appear at the top of the sidebar to open the filesystem browser for adding new workspace folders.
...
#### Scenario: Session in worktree shows worktree name
- **WHEN** a session card is rendered for a session where `isWorktree` is `true`
- **THEN** the card SHALL show "🌲 <folder-name>" (last segment of CWD) instead of "⎇ <branch>"

#### Scenario: Session in normal checkout shows branch
- **WHEN** a session card is rendered for a session where `isWorktree` is `false` or `undefined`
- **THEN** the card SHALL show "⎇ <branch>" as before

#### Scenario: Group header action buttons
- **WHEN** a group header is rendered on localhost
- **THEN** it SHALL show "Add pi-agent" and "Add worktree" action buttons alongside editor buttons

#### Scenario: Group header actions hidden on remote
- **WHEN** the dashboard is accessed via a non-localhost URL
- **THEN** no action buttons SHALL be displayed on group headers

#### Scenario: Empty workspace group displayed
- **WHEN** a DB workspace exists with no active sessions
- **THEN** the sidebar SHALL show the workspace as a group with its name, action buttons, and "(0)" session count

#### Scenario: Add Folder button
- **WHEN** the sidebar is rendered
- **THEN** an "Add Folder" button SHALL appear at the top that opens the filesystem browser

#### Scenario: Remove DB workspace group
- **WHEN** a user clicks "✕ remove" on a DB-backed group header
- **THEN** the workspace SHALL be deleted from the database and the group removed (unless sessions exist)
