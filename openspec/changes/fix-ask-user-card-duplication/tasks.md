...
- [ ] 1.1 In `packages/client/src/components/ChatView.tsx`, compute a set of
...
- [ ] 1.2 In the `toolResult` render branch, before rendering `ToolCallStep`, return
...
- [ ] 2.1 In `ConfirmRenderer.tsx`, add the `params.message` markdown body to the
      ...
      description.
- [ ] 2.2 Same for `SelectRenderer.tsx` resolved branch (below title, above the
      dimmed option list). → verify: answered select shows the description.
- [ ] 2.3 Same for `MultiselectRenderer.tsx` resolved branch. → verify: answered
      multiselect shows the description.
- [ ] 2.4 Same for `InputRenderer.tsx` resolved branch (below title, above the value
      field). → verify: answered input shows the description.
- [ ] 2.5 Leave `cancelled` / `dismissed` compact states message-free. → verify: those
      states unchanged.

## Tests

- [ ] T.1 `ChatView` test: given a message list with a `toolResult`
...
- [ ] T.2 `ChatView` test: given only a `toolResult` (`toolName: "ask_user"`) with no
      paired `interactiveUi` (history-reload case), the tool card IS rendered. →
      verify: reconstructed answer path preserved.
- [ ] T.3 Renderer tests: resolved `ConfirmRenderer` / `SelectRenderer` /
...
- [ ] V.1 `npm test 2>&1 | tee /tmp/pi-test.log` green; existing
      `AskUserToolRenderer.test.tsx` and interactive-renderer tests still pass.
- [ ] V.2 `npm run quality:changed` clean (Biome + tsc + tests).
- [ ] V.3 Manual: trigger a live `ask_user{confirm}` in the dashboard, confirm one
