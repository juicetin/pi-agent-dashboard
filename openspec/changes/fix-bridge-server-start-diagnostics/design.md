...
### Timeout value (open — task 1.1)
Candidates: 8 s, 10 s, 15 s. Rationale for 10 s default: the observed slow host reached `writePid()` and came up "a few seconds" after the 2 s mark; 10 s gives a comfortable margin without making a genuine crash feel hung (and `EarlyExitError` still beats the timeout the instant the child exits, so a real crash surfaces fast regardless). Decide against the slowest supported host class.
...
### Append vs ignore
Switching the bridge from `stdio: "ignore"` to `{ logFile }` means the server log now captures auto-spawn output too. Append-mode + header line is the existing policy; the file is shared with the CLI path, which is fine (one server log, one server). Confirm no rotation requirement (task 1.2) — the server is long-running and the log is low-volume.
