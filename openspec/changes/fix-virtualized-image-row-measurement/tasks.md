...
- [ ] 1.1 On `develop`, run the app and confirm whether a user message with a
...
- [ ] 1.2 Add a test in `packages/client/src/components/__tests__/` that renders
...
- [ ] 2.1 `ChatView.tsx` `ImageAttachments`: add an `onImageLoad` callback prop;
...
- [ ] 2.2 Give the `<img>` a reserved box while loading (`min-height`/`min-width`
  or intrinsic attrs) so the pre-decode measurement is bounded, not ~0px.

## 3. Estimate parity (optional, drift-only)
- [ ] 3.1 If low-risk, make `estimateVirtualRowSize` return a taller value for a
...
## 4. Verify
- [ ] 4.1 New re-measure test passes.
- [ ] 4.2 `npm test 2>&1 | tee /tmp/pi-test.log` green (ChatView + virtual-rows
  suites); `grep -nE 'FAIL|Error|✗' /tmp/pi-test.log` empty.
- [ ] 4.3 `npm run quality:changed` clean.
- [ ] 4.4 Confirm no measure storm on a multi-image message (single measure per
  row per frame) — inspect via a spy count in the test.

## 5. Manual QA (tested later)
- [ ] 5.1 In a live session, paste an image into a message and send; confirm the
...
- [ ] 5.2 Send a message with multiple images; confirm all render and the row
