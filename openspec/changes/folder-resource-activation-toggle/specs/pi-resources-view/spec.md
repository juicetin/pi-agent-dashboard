...
- **GIVEN** a folder with a loose extension `.pi/my-ext.ts` and no exclusion for it in `.pi/settings.json`
- **WHEN** the user disables its row on the folder Resources surface
- **THEN** the client POSTs `/api/resources/toggle` with `{ scope: "local", type: "extension", filePath: ".pi/my-ext.ts", enabled: false }`
- **AND** the folder's `.pi/settings.json#extensions` gains a `-<relPath>` force-exclude entry for that path
...
- **GIVEN** a global loose skill under `~/.pi/agent` with no exclusion
...
- **AND** `~/.pi/agent/settings.json#skills` gains a `-<relPath>` force-exclude entry
- **AND** no folder `.pi/settings.json` is written

#### Scenario: Re-enabling replaces the exclusion with a force-include
- **GIVEN** a settings file whose `extensions` array force-excludes `-.pi/my-ext.ts`
- **WHEN** the user enables that row
- **THEN** the client POSTs `/api/resources/toggle` with `{ enabled: true }`
- **AND** the `-<relPath>` entry is stripped and a `+<relPath>` force-include entry is written to that scope's `extensions` array (matching pi's own config format)
...
- **GIVEN** a folder with a loose skill `.pi/notes.md` and no resource-array rule referencing it
...
- **GIVEN** a folder whose `.pi/settings.json#skills` contains `-./.pi/notes.md`
- **WHEN** the resources are scanned for that cwd
