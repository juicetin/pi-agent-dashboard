...
- [ ] 1.1 Add `"--table-stripe"` to `CSS_VAR_KEYS` in `packages/client/src/lib/themes.ts`.
- [ ] 1.2 Add two shared consts `darkTableVars` / `lightTableVars` and merge them
...
- [ ] 1.3 Add `--table-stripe` to `:root` and `[data-theme="light"]` in
  `packages/client/src/index.css` so the `base` theme resolves it.

## 2. Table CSS

- [ ] 2.1 Replace the `.markdown-content table` block
...
- [ ] 3.1 Add a unit test asserting `--table-stripe` is defined for every theme
  in both modes → verify: test passes.
- [ ] 3.2 Confirm the existing "renders GFM table as HTML table" test in
  `MarkdownContent.test.tsx` stays green → verify: `npm test` for that file.

## 4. Visual verification

- [ ] 4.1 Rebuild client + restart (`npm run build` → `POST /api/restart`),
  render a chat message with a GFM table, confirm striped rows + separated header
  in at least base/dark and one light theme.

## 5. Validate

- [ ] 5.1 `openspec validate markdown-table-styling` passes.
