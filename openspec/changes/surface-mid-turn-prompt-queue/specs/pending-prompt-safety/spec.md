...
The timeout SHALL be **suppressed (paused) while `pendingPrompt.text` is present in `Session.queue.steering` or `Session.queue.followUp`** — that is, while the bridge has acknowledged the prompt by reporting it in the queue. The timeout SHALL (re)start only while the prompt is in flight but not yet queue-acknowledged. If the prompt enters the queue and later leaves the queue (because pi began delivering it) without producing a clearing event within 30 seconds of the leave, the timeout SHALL fire as before.

#### Scenario: Timeout triggers after 30 seconds when prompt is never queued
- **WHEN** `pendingPrompt` is set
- **AND** the prompt's text never appears in `Session.queue.steering` or `Session.queue.followUp`
...
- **WHEN** `pendingPrompt` is set
- **AND** within the 30-second window the text appears in `Session.queue.steering` or `Session.queue.followUp`
...
- **AND** the text is later removed from `Session.queue.steering`/`followUp`
