...
After each terminal agent turn, the bridge asks the naming model (`@naming`, falling back to `@fast`) for a short topic title for the session and applies it once via `pi.setSessionName`. The first successful name ends the loop permanently for that session; an external rename locks naming out permanently. Naming is BOUNDED: a truncated (token-starved) stream is never applied as a name, and `starved` plus `waiting` attempts share one per-session budget, after which naming stops permanently with an actionable error. All model work runs in-process and tolerates failure without crashing or looping.
