...
Guard the dashboard client's `fetch` helpers against non-JSON responses. A reverse-proxy / gateway HTML error page (`<html>…`, 502/504), an SPA-fallback page, an empty body, or a misrouted `/api/*` request must surface as a typed error naming the real HTTP status — never as a cryptic `Unexpected token '<', "<html> <h"... is not valid JSON` parse error that hides the transport failure.
