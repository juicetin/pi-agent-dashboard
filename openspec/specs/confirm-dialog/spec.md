...
Define the `Confirm` preset built on the `Dialog` primitive: a two-button confirmation modal with title/message/optional body content, intent-driven action styling, action wiring, default labels, registry-contract preservation replacing the legacy `ConfirmDialog`, and testId hooks.
