# flow-abort-race Specification

## Purpose

Ensures that aborting a flow with in-flight parallel agent batches unwinds the parent loop immediately rather than waiting for every child `Promise.all` to settle. Threads the AbortSignal into every `spawnAgent` call site so child provider streams can be cancelled at the HTTP layer.
