...
When `handleResumeSession` receives a fork request with `entryId`, it SHALL use `createBranchedSessionFile(sessionFile, entryId)` to produce a pruned JSONL session file containing only entries from root to the target entry, then spawn `pi --fork` on the pruned file.
...
- **THEN** a new JSONL session file is created with only root→"abc-123" entries
