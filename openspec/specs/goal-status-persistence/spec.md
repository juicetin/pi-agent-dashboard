...
Persist goal status and turn progress durably on the `GoalRecord`, projecting live
`goal_status` snapshots onto the owning goal so status and cumulative progress
survive server restarts and remain consistent across drivers.
