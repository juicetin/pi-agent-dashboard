# honcho-server-lifecycle Specification

## Purpose

This capability covers the **self-hosted Honcho server lifecycle** managed by the dashboard plugin: opinionated `docker-compose.yml` template ownership at `~/.honcho/docker-compose.yml`, storage-backend selection, idempotent start/stop/restart over Docker Compose, schema migrations on first boot, health-probe contract, port-conflict and Docker-availability detection, LLM provider configuration (with `pi-model-proxy` as the recommended route), and the aggregate model-listing endpoint. Cloud mode (`mode=cloud`) skips the docker lifecycle entirely.
