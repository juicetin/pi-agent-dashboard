# interactive-cursor-affordance Specification

## Purpose
Guarantee an app-wide pointer-cursor click affordance on enabled interactive controls. Tailwind v4 Preflight dropped the v3 `cursor: pointer` default on `<button>`; a single global `@layer base` rule in the shared `packages/client/src/index.css` restores it across the client, all plugin client bundles, and dialogs/modals. Disabled controls keep the default arrow; explicit `cursor-*` utilities override.
## Requirements
