...
The doctrine artifact SHALL distinguish seeded content from reference-only content by delimiting the seedable sections with stable markers. Only the marked WRITE and READ sections SHALL be copied into the target root `AGENTS.md`; any unmarked section SHALL remain in the shipped artifact, retrievable by kb search but never injected. This keeps per-turn maintenance guidance (how to repair a drifted tree) out of the file that loads on every turn, since root bytes cost tokens on every turn and dilute the rules the model must follow.

#### Scenario: Reference-only doctrine content is not seeded
- **WHEN** the doctrine artifact carries a section outside the seedable markers
- **THEN** the composed block written to the target root `AGENTS.md` SHALL NOT contain that section
- **AND** the section SHALL remain retrievable from the shipped artifact via kb search
