...
Gives the session card a visible "closing" state while a shutdown is in flight,
so the user gets immediate optimistic feedback instead of a card that looks
unchanged until `session_removed` arrives 2-4s later.
