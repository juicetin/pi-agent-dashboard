...
The session list SHALL group sessions by their `cwd` value. Sessions with the same `cwd` SHALL appear together under a group header.
...
- **THEN** the session SHALL be displayed as an inline card with git info shown beneath (no group header)
