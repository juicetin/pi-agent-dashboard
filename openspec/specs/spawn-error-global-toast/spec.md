# Spawn Error Global Toast

## Purpose

Surface every `spawn_error` whose cwd has no on-screen folder banner via the app-level Toast channel, so sibling-worktree / off-screen spawn failures are never silent. Visibility compares cwds via `pathKey(path, platform)` to ignore trailing-slash / separator / case drift.
