...
| `ResourcesPage.tsx` | `usePiResources` tree via MergedScopeSection. Click leaf → onViewFile. Wires `useResourceActivation` → per-row enable/disable toggle + `ResourceReloadBanner` for both Local/Global scopes (change: folder-resource-activation-toggle). See change: directory-settings-page-and-scoped-md-editing. |
