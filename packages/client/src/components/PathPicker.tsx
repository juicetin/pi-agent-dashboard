...
import { parsePathInput } from "@blackbelt-technology/pi-dashboard-shared/platform/paths.js";
 ...
 */
/**
 * Cross-platform wrapper around the shared `parsePathInput` primitive.
 *
 * Platform is inferred from the most-recently-seen `BrowseResult.platform`
 * via the module-level `lastPlatform` ref (default: caller's userAgent guess).
 * This lets the picker correctly split Windows drive-relative paths like
 * `B:Dev` → `{ parent: "B:\\", partial: "Dev" }` instead of develop's
 * POSIX-only behaviour (which would have returned `{ parent: "/", partial:
 * "B:Dev" }` — wrong).
 */
let lastPlatform: NodeJS.Platform = inferPlatform([]);
function parseInput(value: string): { parent: string; partial: string } {
  return parsePathInput(value, lastPlatform);
}
  ...
  const fetchDir = useCallback(
    async (dir: string | undefined, q: string, force = false) => {
      ...
      try {
        ...
        // Track the server-reported platform so parsePathInput splits paths
        // correctly on Windows (drive letters, backslashes) regardless of
        // the client's inferPlatform guess.
        if (result.platform) lastPlatform = result.platform;
