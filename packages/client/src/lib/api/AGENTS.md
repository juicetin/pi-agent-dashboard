...
| `node-runtime-api.ts` | Fetch helpers: `fetchNodeInstalls()` (GET /api/node/installs → candidates + coherence) + `selectNodeRuntime({root, discardHandSet})` (POST — ONE atomic family write). See change: add-node-runtime-family-selection. |
