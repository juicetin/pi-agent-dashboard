...
| `ask-user-tool.ts` | Register `ask_user` pi tool at `session_start`; prompt guidance reserves UI-supplied custom-answer and Select all controls. Exports `registerAskUserTool`. → see `ask-user-tool.ts.AGENTS.md` |
...
| `tui-prompt-adapter.ts` | Production PromptBus adapter for Pi's TUI. Exports `createTuiPromptAdapter`, `TuiPromptUi`. Presents select/input/confirm/editor via captured unpatched UI; select offers collision-safe custom input when available; no multiselect arm. → see `tui-prompt-adapter.ts.AGENTS.md` |
