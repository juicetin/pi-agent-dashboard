...
| `.pi/skills/reverse-spec-from-code/prompts/judge.md` | Offline fitness prompt (spec-vs-spec). Scores a GENERATED spec against a REAL ground-truth spec; oracle is the existing spec, NOT code. Emits strict JSON `{requirement_coverage,scenario_coverage,hallucinated_requirements,missing_requirements,format_compliance,notes}`. Not in the generate flow — `auditor.md` is the runtime gate. Fitness function behind `docs/research/reverse-spec-from-code.md` prompt-tuning + model-loss scores. |
