...
| `test-server.ts` | Boots real `DashboardServer` on OS-assigned ports for integration tests. Exports `createTestServer(overrides)` → `TestServerHandle { server, httpPort, piPort, stop }`. Safe defaults: `host` `127.0.0.1`, `dev` true, `autoShutdown` false, `tunnel` false. Pairs with `setup-home` setupFile for HOME isolation. Defaults `gatewayTcp: true` (production default is socket-only) and still REQUIRES a numeric `piPort`, so a socket-only boot must use `createServer` directly. See change: add-pi-gateway-transport-identity. |
