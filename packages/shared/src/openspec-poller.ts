...
export function buildOpenSpecData(
  ...
  const changes: OpenSpecChange[] = listResult.changes.map((c) => {
    ...
    // Pass-through isComplete from status result when present (absent for null
    // statuses; omitted entirely rather than set to undefined so callers can
    // distinguish "not polled yet" from "polled but not complete").
    if (statusResult && typeof statusResult.isComplete === "boolean") {
      change.isComplete = statusResult.isComplete;
    }
    return change;
  });
