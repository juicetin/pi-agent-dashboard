...
| `discover.ts` | File discovery + idempotency. `resolveInputs` (no arg→`~/Movies`, dir→scan, else explicit files). video {.mkv,.mp4} audio {.m4a,.mp3}. mtime sort oldest-first. `srtPath`/`isTranscribed` skip already-transcribed. `saveSrt`. See change: add-video-transcription-package. |
