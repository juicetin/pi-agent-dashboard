...
| `config.ts` | Resolves `SONIOX_API_KEY` (env first, then gitignored `.env` cwd/skillDir). Parses `MAX_CHUNK_HOURS` (default 4.5) + `MAX_AUDIO_MB` (default 200, 0 disables) + `TRANSCRIBE_CONCURRENCY` (default 8, clamp 1–100). Fails fast on missing key. Exports `loadConfig`, `parseEnvFile`. See change: add-video-transcription-package, parallel-transcription-file-pool. |
...
| `run.ts` | Orchestration core. `run(args, deps)`. config→discover→extract audio if video→`transcribeChunked`→`saveSrt`. Bounded worker pool (`cfg.concurrency`, default 8) over files: dispatch oldest-first, overlap Soniox waits, complete any order. Per-file try/catch. ffmpeg-absent skips video, processes audio. Prints summary. deps injectable for tests. See change: add-video-transcription-package, parallel-transcription-file-pool. |
